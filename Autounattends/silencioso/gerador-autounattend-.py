...
def prettify(elem):
    """Gera XML formatado bonito com declaração UTF-8"""
    ...
    # Remove possível linha vazia no topo
    if pretty_xml.startswith('\n'):
        pretty_xml = pretty_xml[1:]
    return pretty_xml
...
def coletar_configuracoes():
    ...
    if is_mbr:
        ...
        create_recovery = get_input("Criar partição Recovery (450MB antes do Windows)? (S/n)", "s").lower() != "n"
        if create_recovery:
            partitions.append({"order": "2", "type": "Primary", "size": "450",
                               "size_gb": "0.45", "label": "Recovery", "letter": "R", 
                               "typeid": "0x27"})
        
        win_gb = get_input("Tamanho da partição Windows (GB) - Enter = restante", "")
        win_mb = gb_to_mb(win_gb)
        partitions.append({"order": str(len(partitions)+1), "type": "Primary", "size": win_mb, 
                           "size_gb": win_gb or "Restante", "label": "Windows", "letter": "C"})
    else:
        ...
        create_recovery = get_input("Criar partição Recovery (450MB antes do Windows)? (S/n)", "s").lower() != "n"
        if create_recovery:
            partitions.append({"order": "3", "type": "Primary", "size": "450",
                               "size_gb": "0.45", "label": "Recovery", "letter": "R",
